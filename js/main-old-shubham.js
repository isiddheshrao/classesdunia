$(document).ready(function(){
	...
	$('.lp-home-search-input').autocomplete({
        ...
        if(request.term.length === 3 || ajMade.length==0 || ajMade!=request.term.substring(0,3)) {
        	ajMade = request.term.substring(0,3);
	        var options = {
				url : 'search.php',
				data : {
					'term': request.term
			...
			var matcher = new RegExp( $.ui.autocomplete.escapeRegex(request.term), "i" );
			 ...
			 for(var i in iData)
			 {
			 	if(request.term && matcher.test(iData[i]))
			 	{
			 		results.push(iData[i]);
			 	}
			 } 
